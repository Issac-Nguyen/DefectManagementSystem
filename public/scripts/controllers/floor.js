...
angular.module('angularTokenAuthApp.controllers')
    .controller('FloorController', ['$scope', '$state', '$q', '$http', 'Utils', 'Auth', '$modal', 'uiGridConstants', 'buildings', 'floors',
        function($scope, $state, $q, $http, Utils, Auth, $modal, uiGridConstants, buildingsList, floorsList) {
            // $scope.floorsList = floorsList;
            ...
            function getPage() {
                ...
                var data = floorsList.splice(paginationOptions.pageNumber - 1, paginationOptions.pageSize);
                $scope.gridOptions.totalItems = floorsList.length;
            ...
            $scope.onBtnAddNew = function() {
                $scope.HeaderModal = "New Floor";
                action = "New";
                openModal();
            }

            $scope.editRow = function(grid, row) {
                $scope.HeaderModal = "Edit Floor";
                resetVar();
            ...
            $scope.onSubmit = function(form) {
                ...
                var url = action == "New" ? "/webapi/addFloor" : "/webapi/updateFloor"
                if (form.$valid) {
                    console.log('valid');
                    $http({
                        method: 'POST',
            ...
            $scope.deleteRow = function(grid, row) {
                ...
                $scope.HeaderModal = 'Delete Floor';
                modal = $modal.open({
                    animation: true,
